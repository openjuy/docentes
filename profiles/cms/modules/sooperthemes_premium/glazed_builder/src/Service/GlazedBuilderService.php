      ...
      if ($cache = $this->cacheBackend->get('glazed_builder:cms_view_elements_settings')) {
        ...
        foreach ($views as $view) {
          ...
          foreach ($executable_view->displayHandlers as $id => $display) {
            ...
            if ($executable_view->exposed_widgets) {
              $hasExposed = 1;
            }

            $ajaxEnabled = 0;
            if (isset($display->options['use_ajax'])) {
              $ajaxEnabled = ($display->options['use_ajax']);
            }
            elseif (isset($defaultDisplay['display_options']['use_ajax']) && ($defaultDisplay['display_options']['use_ajax'])) {
              $ajaxEnabled = 1;
            }
