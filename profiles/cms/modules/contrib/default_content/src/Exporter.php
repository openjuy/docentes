...
class Exporter implements ExporterInterface {
  ...
  /**
   * An array to keep track of which UUIDs have already been treated for export.
   * This prevents the same entity from being exported multiple times when using
   * recursive export.
   *
   * @var array
   */
  protected $uuids_exported = [];
  ...
  public function exportContentWithReferences($entity_type_id, $entity_id) {
    ...
    foreach ($entities as $entity) {
      if (!in_array($entity->uuid(), $this->uuids_exported)) {
        ...
        $this->uuids_exported[] = $entity->uuid();
      }
    }
  ...
  public function exportModuleWithReferences($module_name) {
    ...
    foreach ($info['default_content'] as $entity_type => $uuids) {
      foreach ($uuids as $uuid) {
        $entity = $this->entityRepository->loadEntityByUuid($entity_type, $uuid);
        $elements = $this->exportContentWithReferences($entity_type, $entity->id());
        $exported_content = array_merge_recursive($exported_content,$elements);
      }
    }
    return $exported_content;
  }
