...
class Importer implements ImporterInterface {
  ...
  public function importContent($module) {
    ...
    if (file_exists($folder)) {
      ...
      $langs_available = array_keys($this->languageManager->getLanguages());
      ...
      foreach ($sorted as $link => $details) {
        if (!empty($file_map[$link])) {
          ...
          $contents = $this->parseFile($file);
          // Decode check language, if language unavailable set to default language
          $decoded = $this->serializer->decode($contents, 'hal_json');
          if (isset($decoded['langcode']) && !in_array($decoded['langcode'][0]['value'], $langs_available)) {
            $decoded['langcode'][0]['value'] = $current_lang;
          }
          array_walk_recursive($decoded, function (&$value, $key, $userdata) {
            if (($key == 'lang') && (!in_array($value, $userdata[1]))) {
              $value = $userdata[0];
            }
          }, [$current_lang, $langs_available]);
